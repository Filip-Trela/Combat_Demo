...
@onready var player = get_parent().get_parent().get_node("Player_combat")
...
func _ready():
	#print(weaknesses)
	navi.path_desired_distance = 1
	navi.target_desired_distance = 1



func _process(_delta):
	#print(anim["parameters/Transition/current_state"])
	...
	if PlayerInfo.is_moving:
		...
		anim["parameters/TimeScale/scale"] = 1
	else:
		anim["parameters/TimeScale/scale"] = 0
