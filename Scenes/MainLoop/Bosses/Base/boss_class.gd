...
@onready var healthbar_pl = preload("res://Scenes/MainLoop/Bosses/Base/HealthBar/Boss_Healthbar.tscn")
var healthbar
var menuC
...
func _ready():
	menuC = get_parent().get_parent().get_parent().get_parent().get_parent()
	menuC = menuC.get_node("CombatMenu")
	
	healthbar = healthbar_pl.instantiate()
	healthbar.boss = self
	menuC.add_child(healthbar)
